...
{
    public class WasmMqttWebSocketChannel : IMqttChannel
    {
        private readonly MqttClientWebSocketOptions _options;

        private SemaphoreSlim _sendLock = new SemaphoreSlim(1, 1);
        private WasmWebSocket _webSocket;

        public WasmMqttWebSocketChannel(MqttClientWebSocketOptions options)
        {
            _options = options ?? throw new ArgumentNullException(nameof(options));
        }

        public WasmMqttWebSocketChannel(WasmWebSocket webSocket, string endpoint, bool isSecureConnection, X509Certificate2 clientCertificate)
        {
        ...
        public async Task ConnectAsync(CancellationToken cancellationToken)
        {
            ...
            var clientWebSocket = new WasmWebSocket();
